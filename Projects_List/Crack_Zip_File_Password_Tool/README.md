...
**DISCLAIMER**: PLEASE USE THIS TOOL FOR ETHICAL PURPOSES ONLY.

## Overview
This project assists users in cracking the password of their encrypted zip files. It does not rely on third-party modules; instead, it utilizes only pre-built ones. This project is fully based on the Brute Force Attack method, although it is a time-consuming tool for cracking large passwords, however it serves as an illustrative example of **CyberSecurity**. Currently, this version only supports cracking zip passwords, but additional features may be added in the future.

The structure of this tool involves first **importing important pre-built modules**, followed by defining two functions: **get_next_series_number()** and **crack_zip_file_password()**.

Here is the code for the `get_next_series_number()` function:
```python
...
The `get_next_series_number` function examines the folder names within a specified directory (**extract_folder**), extracting series numbers from these folder names by considering only numeric prefixes. It then returns the next available series number by finding the highest existing number and adding 1. If no series numbers are found, it suggests starting with 1.

Here is the code for the `crack_zip_file_password()` function:
```python
...
The `crack_zip_file_password()` function attempts to crack an encrypted ZIP file (**zip_file**) using brute force. It iterates through various combinations of characters, trying different password lengths and character sets until it successfully extracts the ZIP contents or reaches the maximum password length. It creates a new directory to store extracted files and logs each extraction attempt. The function returns True if successful, False otherwise.

## How To Use This Project
If you wish to use this project, you can do so manually in your environment by copying and running the code provided below.

## Full Code
Here is the full code to crack an encrypted zip file:
```python
#####|| Configuration ||#####
...
It is also available with a user input method:
```python
# IMPORT PRE-BUILT MODULES
try:		
    import os
